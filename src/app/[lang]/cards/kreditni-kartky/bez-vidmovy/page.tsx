...
import { CreditCard, Filter, Star, TrendingUp, Gift, Percent, Calendar, Shield, Sparkles, Award, CheckCircle, Zap } from 'lucide-react';
...
export default function CreditCardsNoRefusalPage({
  params,
}: {
  params: { lang: Locale };
}) {
  ...
  const sortedCards = [...filteredCards].sort((a, b) => {
    if (sortBy === 'approval') {
      const aRate = parseFloat(a.approvalRate?.replace(/[^\d.]/g, '') || '0');
      const bRate = parseFloat(b.approvalRate?.replace(/[^\d.]/g, '') || '0');
      return bRate - aRate;
    }
    if (sortBy === 'cashback') {
      ...
      <div className="bg-white/80 backdrop-blur-lg border-b border-gray-200/50">
        <div className="container-custom py-4">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            <span className="text-gray-400">/</span>
            <span className="text-slate-900 font-semibold">{dict.creditCardsNoRefusal.breadcrumb}</span>
          </div>
        </div>
      </div>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              >
                <span className="relative z-10 flex items-center gap-2">
                  ...
                  {dict.creditCardsNoRefusal.viewRating}
                </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
            ...
            <aside className={`lg:col-span-3 space-y-6 ${showFilters ? '' : 'hidden lg:block'}`}>
              ...
              <div className="sticky top-8">
                ...
                <div className="mt-6 bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 border-2 border-green-100/50 shadow-lg">
                  <h3 className="font-black mb-4 flex items-center gap-2 text-slate-900 uppercase tracking-wide text-sm">
                    <CheckCircle className="w-5 h-5 text-green-600" />
                    {dict.creditCardsNoRefusal.highApproval}
                  </h3>
                  <div className="text-sm text-slate-700 space-y-2 font-medium">
                    <p className="flex items-start gap-2">
                      <span className="text-green-600 mt-0.5">✓</span>
                      <span>{dict.creditCardsNoRefusal.fastDecision}</span>
                    </p>
                    <p className="flex items-start gap-2">
                      <span className="text-green-600 mt-0.5">✓</span>
                      <span>{dict.creditCardsNoRefusal.minRequirements}</span>
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-6">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <option value="approval">{dict.creditCardsNoRefusal.byApproval}</option>
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-3 flex-wrap">
                                {card.highApproval && card.approvalRate && (
                                  <div className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-4 py-2 rounded-full font-black shadow-lg">
                                    <Zap className="w-4 h-4" />
                                    <span>{dict.creditCardsNoRefusal.approval}: {card.approvalRate}</span>
                                  </div>
                                )}
                              </div>
            ...
            <article className="prose prose-lg max-w-none">
              <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-100 p-8 md:p-12">
                <div className="space-y-6 text-gray-700 leading-relaxed">
                  <p className="text-lg">
                    {dict.creditCardsNoRefusal.article.intro}
                  </p>

                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsNoRefusal.article.canGetTitle}
                  </h2>
                  <p>{dict.creditCardsNoRefusal.article.canGet}</p>
                  <p>{dict.creditCardsNoRefusal.article.canGetParagraph2}</p>

                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsNoRefusal.article.howToApplyTitle}
                  </h2>
                  <p>{dict.creditCardsNoRefusal.article.howToApplyIntro}</p>
                  <ul className="space-y-3 ml-6">
                    {dict.creditCardsNoRefusal.article.howToApply.map((step: string, idx: number) => (
                  ...
                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsNoRefusal.article.withLimitTitle}
                  </h2>
                  <p>{dict.creditCardsNoRefusal.article.withLimit}</p>
                  <p>{dict.creditCardsNoRefusal.article.withLimitParagraph2}</p>

                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsNoRefusal.article.advantagesTitle}
                  </h2>
                  <ul className="space-y-3 ml-6">
                    {dict.creditCardsNoRefusal.article.advantages.map((advantage: string, idx: number) => (
                      <li key={idx} className="flex items-start gap-3">
                        <span className="text-primary font-black mt-1">•</span>
                        <span>{advantage}</span>
                      </li>
                  ...
                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsNoRefusal.article.conclusionTitle}
                  </h2>
                  <p className="text-lg font-medium">
                    {dict.creditCardsNoRefusal.article.conclusion}
                  </p>
                </div>
              </div>
            </article>
