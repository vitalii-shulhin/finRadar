...
type SortOption = 'rating' | 'cashback' | 'limit' | 'gracePeriod';

const getRankBadge = (position?: number) => {
  if (position === 1) {
    return (
      <div className="flex items-center gap-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white px-4 py-2 rounded-full font-black shadow-lg">
        <Trophy className="w-5 h-5" />
        <span className="text-lg">1 місце</span>
      </div>
    );
  } else if (position === 2) {
    return (
      <div className="flex items-center gap-2 bg-gradient-to-r from-gray-400 to-gray-500 text-white px-4 py-2 rounded-full font-black shadow-lg">
        <Medal className="w-5 h-5" />
        <span className="text-lg">2 місце</span>
      </div>
    );
  } else if (position === 3) {
    return (
      <div className="flex items-center gap-2 bg-gradient-to-r from-orange-600 to-orange-700 text-white px-4 py-2 rounded-full font-black shadow-lg">
        <Medal className="w-5 h-5" />
        <span className="text-lg">3 місце</span>
      </div>
    );
  }
  return (
    <div className="flex items-center gap-2 bg-gradient-to-r from-slate-600 to-slate-700 text-white px-4 py-2 rounded-full font-black shadow-lg">
      <span className="text-lg">#{position}</span>
    </div>
  );
};

export default function CreditCardsRatingPage({
  params,
}: {
  params: { lang: Locale };
}) {
  ...
  const sortedCards = [...filteredCards].sort((a, b) => {
    ...
    if (sortBy === 'gracePeriod') {
      return (b.gracePeriod || 0) - (a.gracePeriod || 0);
    }
    return 0;
  });
      ...
      <div className="bg-white/80 backdrop-blur-lg border-b border-gray-200/50">
        <div className="container-custom py-4">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            <span className="text-gray-400">/</span>
            <Link href={`/${params.lang}/cards/${params.lang === 'uk' ? 'kreditni-kartky' : 'kreditnye-karty'}`} className="text-gray-600 hover:text-primary transition-colors">
              {dict.creditCards.breadcrumb}
            </Link>
            <span className="text-gray-400">/</span>
            <span className="text-slate-900 font-semibold">{dict.creditCardsRating.breadcrumb}</span>
          </div>
        </div>
      </div>
          ...
          <div className="max-w-4xl">
            ...
            <div className="inline-flex items-center gap-3 mb-6">
              ...
              <span className="text-sm font-bold text-primary uppercase tracking-wider flex items-center gap-2">
                ...
                {dict.creditCardsRating.eyebrow}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              >
                {dict.creditCardsRating.allCreditCards}
              </Link>
            </div>
          </div>
        </div>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-6">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <option value="rating">{dict.creditCardsRating.byPosition}</option>
                        <option value="cashback">{dict.cards.byCashback}</option>
                        <option value="limit">{dict.cards.byLimit}</option>
                        <option value="gracePeriod">{dict.creditCardsRating.byGracePeriod}</option>
                      </select>
                    </div>
                  </div>
                  <button
                      ...
                      <div className="p-8">
                        ...
                        <div className="flex items-start justify-between mb-6">
                          <div className="flex items-start gap-6 flex-1">
                            <div className={`relative w-20 h-20 bg-gradient-to-br ${card.color} rounded-2xl flex items-center justify-center shadow-2xl transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 overflow-hidden`}>
                              <Image
                                src={card.bankLogo}
                                alt={`${card.bank} logo`}
                                width={144}
            ...
            <article className="prose prose-lg max-w-none">
              <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-100 p-8 md:p-12">
                <div className="space-y-6 text-gray-700 leading-relaxed">
                  <p className="text-lg">
                    {dict.creditCardsRating.article.intro}
                  </p>

                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsRating.article.ratingTitle}
                  </h2>
                  <p>{dict.creditCardsRating.article.rating}</p>
                  <p>{dict.creditCardsRating.article.ratingParagraph2}</p>

                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsRating.article.howToApplyTitle}
                  </h2>
                  <p>{dict.creditCardsRating.article.howToApply}</p>
                  <p>{dict.creditCardsRating.article.howToApplyParagraph2}</p>

                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsRating.article.withLimitTitle}
                  </h2>
                  <p>{dict.creditCardsRating.article.withLimit}</p>
                  <p>{dict.creditCardsRating.article.withLimitParagraph2}</p>

                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsRating.article.advantagesTitle}
                  </h2>
                  <ul className="space-y-3 ml-6">
                    {dict.creditCardsRating.article.advantages.map((advantage: string, idx: number) => (
                  ...
                  <h2 className="text-3xl font-black text-slate-900 mt-10 mb-6 flex items-center gap-3">
                    <span className="text-primary">🔹</span>
                    {dict.creditCardsRating.article.conclusionTitle}
                  </h2>
                  <p className="text-lg font-medium">
                    {dict.creditCardsRating.article.conclusion}
                  </p>
                </div>
              </div>
            </article>
