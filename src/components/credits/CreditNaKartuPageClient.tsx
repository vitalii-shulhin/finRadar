...
interface CreditNaKartuPageClientProps {
  lang: Locale;
}

export default function CreditNaKartuPageClient({ lang }: CreditNaKartuPageClientProps) {
      ...
      <div className="bg-white border-b">
        <div className="container-custom py-4">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>/</span>
            <span className="text-gray-900">{lang === 'uk' ? 'Кредит на карту' : 'Кредит на карту'}</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-4xl font-bold font-heading mb-2">
                {lang === 'uk' ? 'Кредит на карту' : 'Кредит на карту'} 2026
              </h1>
              <p className="text-xl text-blue-100">
                {sortedLoans.length} {lang === 'uk' ? 'кредитних пропозицій на вигідних умовах' : 'кредитных предложений на выгодных условиях'}
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 overflow-x-auto">
            <div className="flex gap-2 min-w-max">
              ...
              <Link
                ...
                className="bg-white/10 backdrop-blur-sm text-white hover:bg-white/20 px-4 py-2 rounded-lg font-semibold whitespace-nowrap transition-colors"
              >
                {lang === 'uk' ? 'Кредитні картки' : 'Кредитные карты'}
              </Link>
            </div>
          </div>
        </div>
            ...
            {showCalculator && (
              <div className="mt-4 pt-4 border-t">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-semibold mb-3">
                      {lang === 'uk' ? 'Термін кредиту (днів)' : 'Срок кредита (дней)'}
                    </label>
                    <input
                      type="range"
                      min="1"
                    ...
                    />
                    <div className="flex justify-between items-center mt-2">
                      <span className="text-sm text-gray-600">{lang === 'uk' ? '1 день' : '1 день'}</span>
                      <div className="text-2xl font-bold text-primary">
                        {loanTerm} {lang === 'uk' ? (loanTerm === 1 ? 'день' : loanTerm < 5 ? 'дні' : 'днів') : (loanTerm === 1 ? 'день' : loanTerm < 5 ? 'дня' : 'дней')}
                      </div>
                      <span className="text-sm text-gray-600">{lang === 'uk' ? '365 днів' : '365 дней'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-gradient-to-br from-primary to-primary-dark text-white p-4 rounded-lg">
                    <div className="text-sm opacity-90 mb-1">{lang === 'uk' ? 'Сума кредиту' : 'Сумма кредита'}</div>
          ...
          <aside className={`lg:w-80 space-y-6 ${showFilters ? 'block' : 'hidden lg:block'}`}>
            ...
            <div className="card p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                >
                  {lang === 'uk' ? 'Скинути все' : 'Сбросить все'}
                </button>
              </div>
              ...
              <div className="mb-6">
                <h3 className="font-semibold mb-3 flex items-center justify-between">
                  <span>{lang === 'uk' ? 'Відсоткова ставка (% на день)' : 'Процентная ставка (% в день)'}</span>
                  {(minRate !== 0 || maxRate !== 2) && (
                    <button
                      onClick={() => {
                        setMinRate(0);
                ...
                <div className={`space-y-3 p-3 rounded-lg border-2 transition-colors ${
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-sm text-gray-600">{lang === 'uk' ? 'Від' : 'От'}</label>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-sm text-gray-600">{lang === 'uk' ? 'До' : 'До'}</label>
              ...
              <div>
                <h3 className="font-semibold mb-3">
                  {lang === 'uk' ? 'Кредитор' : 'Кредитор'}
                  {selectedLenders.length > 0 && (
                    <span className="ml-2 text-xs text-primary">
                      ({selectedLenders.length} {lang === 'uk' ? 'обрано' : 'выбрано'})
                    </span>
                  )}
                </h3>
            ...
            <div className="card p-6 bg-blue-50 border-blue-200">
              ...
              <div className="text-sm text-gray-700 space-y-2">
                <p>✓ {lang === 'uk' ? 'Всі кредитори ліцензовані НБУ' : 'Все кредиторы лицензированы НБУ'}</p>
                <p>✓ {lang === 'uk' ? 'Відсотки розраховуються щодня' : 'Проценты рассчитываются ежедневно'}</p>
                <p>✓ {lang === 'uk' ? 'Перший кредит часто безкоштовний' : 'Первый кредит часто бесплатный'}</p>
                <p>⚠️ {lang === 'uk' ? 'Не беріть більше, ніж можете повернути' : 'Не берите больше, чем можете вернуть'}</p>
              </div>
            </div>
          </aside>
          ...
          <div className="flex-1">
            ...
            {hasActiveFilters && (
              <div className="card p-4 mb-4">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-sm text-gray-600 font-medium">{lang === 'uk' ? 'Активні фільтри:' : 'Активные фильтры:'}</span>
                  ...
                  {(minRate !== 0 || maxRate !== 2) && (
                    ...
                    >
                      {lang === 'uk' ? 'Ставка' : 'Ставка'}: {minRate}% - {maxRate}%
                      <span className="text-xs">✕</span>
                    </button>
                  )}
                  <button
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4 flex-wrap">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-600">{lang === 'uk' ? 'Знайдено:' : 'Найдено:'}</span>
                    <span className="text-lg font-bold text-primary">
                      {sortedLoans.length} {lang === 'uk' ? (sortedLoans.length === 1 ? 'пропозиція' : sortedLoans.length < 5 ? 'пропозиції' : 'пропозицій') : (sortedLoans.length === 1 ? 'предложение' : sortedLoans.length < 5 ? 'предложения' : 'предложений')}
                    </span>
                  </div>
                  <div className="h-6 w-px bg-gray-300"></div>
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-600">{lang === 'uk' ? 'Сортувати:' : 'Сортировать:'}</span>
                    <select
                      value={sortBy}
                      onChange={(e) => setSortBy(e.target.value as SortOption)}
                      className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
                    >
                      <option value="recommended">{lang === 'uk' ? 'Рекомендовані' : 'Рекомендуемые'}</option>
                      <option value="rate">{lang === 'uk' ? 'За ставкою' : 'По ставке'}</option>
                      <option value="amount">{lang === 'uk' ? 'За максимальною сумою' : 'По максимальной сумме'}</option>
                      <option value="approval">{lang === 'uk' ? 'За швидкістю схвалення' : 'По скорости одобрения'}</option>
                      <option value="time">{lang === 'uk' ? 'За часом видачі' : 'По времени выдачи'}</option>
                    </select>
                  </div>
                </div>
                <button
                ...
                >
                  ...
                  {showFilters ? (lang === 'uk' ? 'Сховати фільтри' : 'Скрыть фильтры') : (lang === 'uk' ? 'Показати фільтри' : 'Показать фильтры')}
                </button>
              </div>
            </div>
            ...
            {sortedLoans.length === 0 ? (
              <div className="card p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">{lang === 'uk' ? 'Нічого не знайдено' : 'Ничего не найдено'}</h3>
                <p className="text-gray-600 mb-4">
                  {lang === 'uk' ? 'За заданими параметрами не знайдено жодної пропозиції' : 'По заданным параметрам не найдено ни одного предложения'}
                </p>
                <div className="text-sm text-gray-600 mb-4">
                  <p>{lang === 'uk' ? 'Поточні параметри:' : 'Текущие параметры:'}</p>
                  <ul className="mt-2 space-y-1">
                    <li>{lang === 'uk' ? 'Сума:' : 'Сумма:'} <strong>{loanAmount.toLocaleString()} ₴</strong></li>
                    <li>{lang === 'uk' ? 'Термін:' : 'Срок:'} <strong>{loanTerm} {lang === 'uk' ? 'днів' : 'дней'}</strong></li>
                    <li>{lang === 'uk' ? 'Ставка:' : 'Ставка:'} <strong>{minRate}% - {maxRate}%</strong></li>
                    {selectedLenders.length > 0 && (
                      <li>{lang === 'uk' ? 'Кредитори:' : 'Кредиторы:'} <strong>{selectedLenders.join(', ')}</strong></li>
                    )}
                  </ul>
                </div>
                <button
                ...
                >
                  {lang === 'uk' ? 'Скинути всі фільтри' : 'Сбросить все фильтры'}
                </button>
              </div>
            ) : (
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              <div className="flex items-center gap-3 text-sm">
                                ...
                                <div className="flex items-center gap-1 text-green-600">
                                  ...
                                  <span>{loan.approvalRate}% {lang === 'uk' ? 'схвалень' : 'одобрений'}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                          <div className="bg-blue-50 p-3 rounded-lg">
                            <div className="text-xs text-gray-600 mb-1">{lang === 'uk' ? 'Сума' : 'Сумма'}</div>
                        ...
                        <div className="bg-gradient-to-r from-primary to-primary-dark text-white p-4 rounded-lg mb-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="text-sm opacity-90 mb-1">{lang === 'uk' ? 'Орієнтовний платіж на місяць' : 'Ориентировочный платеж в месяц'}</div>
                              <div className="text-3xl font-bold">{monthlyPayment.toLocaleString()} ₴</div>
                            </div>
                            <Smartphone className="w-12 h-12 opacity-20" />
                          </div>
                          <div className="text-xs opacity-75 mt-2">
                            * {lang === 'uk' ? 'Розрахунок приблизний. Точні умови дізнавайтесь у кредитора' : 'Расчет приблизительный. Точные условия узнавайте у кредитора'}
                          </div>
                        </div>
          ...
          <div className="max-w-4xl mx-auto prose prose-lg">
            ...
            <div className="mb-8">
              <p className="text-gray-700 leading-relaxed mb-4">
                {dict.creditNaKartuArticle.intro}
              </p>
              <p className="text-gray-700 leading-relaxed">
                {dict.creditNaKartuArticle.introParagraph2}
              </p>
            </div>
            ...
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                {dict.creditNaKartuArticle.howToGetTitle}
              </h2>
              <p className="text-gray-700 leading-relaxed mb-4">
                {dict.creditNaKartuArticle.howToGetContent}
              </p>
              <p className="text-gray-700 leading-relaxed">
                {dict.creditNaKartuArticle.howToGetParagraph2}
              </p>
            </section>
            ...
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                {dict.creditNaKartuArticle.noRefusalTitle}
              </h2>
              <p className="text-gray-700 leading-relaxed mb-4">
                {dict.creditNaKartuArticle.noRefusalContent}
              </p>
              <p className="text-gray-700 leading-relaxed">
                {dict.creditNaKartuArticle.noRefusalParagraph2}
              </p>
            </section>
            ...
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                {dict.creditNaKartuArticle.badCreditTitle}
              </h2>
              <p className="text-gray-700 leading-relaxed mb-4">
                {dict.creditNaKartuArticle.badCreditContent}
              </p>
              <p className="text-gray-700 leading-relaxed">
                {dict.creditNaKartuArticle.badCreditParagraph2}
              </p>
            </section>
            ...
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                {dict.creditNaKartuArticle.urgentCreditTitle}
              </h2>
              <p className="text-gray-700 leading-relaxed mb-4">
                {dict.creditNaKartuArticle.urgentCreditContent}
              </p>
              <p className="text-gray-700 leading-relaxed">
                {dict.creditNaKartuArticle.urgentCreditParagraph2}
              </p>
            </section>
            ...
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                {dict.creditNaKartuArticle.typesTitle}
              </h2>
              <p className="text-gray-700 leading-relaxed mb-4">
                {dict.creditNaKartuArticle.typesContent}
              </p>
              <ul className="list-disc list-inside space-y-2 mb-4">
                {dict.creditNaKartuArticle.typesList.map((type, index) => (
              ...
              <p className="text-gray-700 leading-relaxed">
                {dict.creditNaKartuArticle.typesParagraph2}
              </p>
            </section>
            ...
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                {dict.creditNaKartuArticle.advantagesTitle}
              </h2>
              <p className="text-gray-700 leading-relaxed mb-4">
                {dict.creditNaKartuArticle.advantagesContent}
              </p>
              <ul className="list-disc list-inside space-y-2 mb-4">
                {dict.creditNaKartuArticle.advantagesList.map((advantage, index) => (
                  <li key={index} className="text-gray-700 leading-relaxed">
                    {advantage}
                  </li>
                ))}
              ...
              <p className="text-gray-700 leading-relaxed">
                {dict.creditNaKartuArticle.advantagesParagraph2}
              </p>
            </section>
            ...
            <section className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                {dict.creditNaKartuArticle.conclusionTitle}
              </h2>
              <p className="text-gray-700 leading-relaxed">
                {dict.creditNaKartuArticle.conclusionContent}
              </p>
            </section>
          </div>
        </article>
