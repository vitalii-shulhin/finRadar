...
import { CREDIT_OFFERS_BASE } from '@/data/creditOffers';
  ...
  return (
    <section className="bg-white py-6 sm:py-12">
      <div className="container-custom">
          ...
          {services.map((service, index) => {
            ...
            return (
              ...
              >
                {/* Badge */}
                {service.badge && (
                  <span className="absolute top-1 right-1 sm:top-3 sm:right-3 bg-red-500 text-white text-[9px] sm:text-xs font-semibold px-1 sm:px-2 py-0.5 sm:py-1 rounded">
                    {service.badge}
                  </span>
                )}

                {/* Icon */}
                <div className={`${service.color} w-16 h-16 sm:w-14 sm:h-14 rounded-lg sm:rounded-xl flex items-center justify-center mb-2 sm:mb-4 group-hover:scale-110 transition-transform duration-300`}>
        ...
        {/* Top Credit Offers */}
        <div className="mt-12">
          <h3 className="text-xl sm:text-2xl font-bold font-heading mb-4 sm:mb-6">{dict.home.services.popularCredits}</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-6">
            {CREDIT_OFFERS_BASE.slice(0, 5).map((offer, index) => (
              <Link
                key={index}
                href={offer.creditUrl || '#'}
                target="_blank"
                rel="noopener noreferrer"
                className="card overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group"
              >
                <div className={`bg-gradient-to-br ${offer.color} p-3 sm:p-4 flex items-center justify-center`}>
                  <div className="bg-white rounded-lg p-1 sm:p-2 w-full h-14 sm:h-16 flex items-center justify-center">
                    <Image
                      src={offer.logo}
                      alt={offer.bank}
                      width={60}
                ...
                <div className="p-3 sm:p-4">
                  <h4 className="text-xs sm:text-base font-semibold mb-1 sm:mb-2 group-hover:text-primary transition-colors leading-tight">
                    {offer.bank}
                  </h4>
                  <div className="flex items-center gap-1 sm:gap-2 mb-1 sm:mb-2">
                    <span className="text-yellow-500 text-xs">★</span>
                    <span className="text-[10px] sm:text-sm font-medium">{offer.rating}</span>
                    <span className="text-[9px] sm:text-xs text-gray-500">({offer.reviews})</span>
                  </div>
                  <div className="text-[10px] sm:text-sm text-gray-600 space-y-0.5 sm:space-y-1">
                    <p className="hidden sm:block">Amount: {offer.minAmount} - {offer.maxAmount} ₴</p>
                    <p className="hidden sm:block">Rate: {offer.minRate}%</p>
                    <p className="hidden sm:block">Term: {offer.minTerm} - {offer.maxTerm} days</p>
                    <p className="sm:hidden">{offer.minAmount} - {offer.maxAmount} ₴</p>
                  </div>
                </div>
              </Link>
            ))}
